...
## 10) Обязательное правило для следующей нейронки

После каждого завершенного и проверенного изменения нужно делать commit и push в `main`.

Минимальный цикл на каждую задачу:
1. Внести изменения
2. Прогнать проверку (`npm run build`, и при необходимости доп. тесты)
3. Сделать `git add ...`
4. Сделать `git commit -m "..."`
5. Сделать `git push origin main`

Не копить много незапушенных изменений в рабочем дереве.
