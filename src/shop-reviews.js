...
export function buildEmptyShopReview() {
  const lines = [
    "«На прилавках опять пусто! Зачем открываться, если нечего продавать?»",
    "«Пришёл за рудой — ушёл с пустыми руками. Позор.»",
    "«Магазин без товара — это просто комната с табуреткой.»",
    "«Зашёл, посмотрел на пустые полки, развернулся. Время потрачено.»",
    "«Опять ничего нет. Может, шахтёру стоит добывать, а не считать монеты?»",
    "«Товар кончился? Может и лавку закроете тогда?»",
    "«Пустые прилавки и скучающий продавец. Грустное зрелище.»",
    "«Полки пустые, а вывеска висит. Обман какой-то.»",
  ];
  return {
    text: pick(lines),
    stars: 1,
